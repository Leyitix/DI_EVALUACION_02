...
export class ElementoLista extends Component {
  ...
  render() {
    ...
    return (
      ...
      <Pressable
        elevation={4}
        onPress={this.props.comunicaAlPadreElementoSeleccionado.bind(
          this,
          element,
        )}
        ...
        <View
          ...
          <View>
            <Text
              style={styles.text}
              onPress={this.elementoPulsado.bind(this, element)}>
              {element.profe}
            </Text>
          </View>
          <View>
            <Text style={[styles.text, {color: 'black'}]}>
              {element.modulo}
              <Badge value={element.horas + ' h.'} status="error" />
            </Text>
          </View>
        </View>
        <View style={{backgroundColor: 'orange', alignItems: 'center'}}>
          <Text style={{color: 'blue'}}>{element.email}</Text>
        </View>
      </Pressable>
    );
  }
